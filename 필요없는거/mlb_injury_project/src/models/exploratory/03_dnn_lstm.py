"""
모델 3. LSTM 기반 DNN, 다중분류(0=안다침/1=어깨/2=팔꿈치/3=그외)
*** GPU/torch가 설치된 별도 가상환경에서 실행할 것 ***
입력: data/processed/{role}_sequence_arrays.npz (경기 순서를 보존한 시계열)

실행: python 03_dnn_lstm.py --role bullpen
      python 03_dnn_lstm.py --role starter
"""
import argparse
import sys
...
from common_dnn import (  # noqa: E402
    DEVICE, MODEL_DIR, InjuryLSTM, evaluate, load_role_arrays, make_datasets,
)


def run(role: str, epochs: int = 100, batch_size: int = 512, lr: float = 1e-3, patience: int = 10):
    data, meta = load_role_arrays(role, exclude_other=False)
    ...
    print(f"[모델3] LSTM 다중분류 - {role.upper()}  device={DEVICE}")
    print(f"train={len(datasets['train']):,} val={len(datasets['val']):,} test={len(datasets['test']):,}")
    print(f"class weights: {dict(zip([0, 1, 2, 3], class_weights.round(2)))}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        from common_dnn import eval_val_auc_only
    ...
    model_path = MODEL_DIR / f"03_{role}_dnn_lstm.pt"
