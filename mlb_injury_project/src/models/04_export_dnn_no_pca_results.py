"""
04_dnn_no_pca_bayesian.py로 학습된 모델(3class/binary 둘 다)을 다시 불러와서 결과를
엑셀 파일 하나에 정리한다. 다른 export 스크립트들과 시트 구성을 동일하게 맞춰서
02번(PCA 적용 DNN)과 나란히 비교하기 쉽게 했다 - 이게 이 04번을 만든 목적이다.
*** GPU/torch가 설치된 별도 가상환경에서 실행할 것 ***

출력: models/04_dnn_no_pca_bayesian_results.xlsx
"""
import json
import sys
...
def main():
    ...
    for role in ("bullpen", "starter"):
        for label_mode in ("3class", "binary"):
            summary = json.loads(
                (MODEL_DIR / f"04_{role}_dnn_no_pca_bayesian_{label_mode}_summary.json").read_text(encoding="utf-8")
            )
            ...
            model.load_state_dict(
                torch.load(MODEL_DIR / f"04_{role}_dnn_no_pca_bayesian_{label_mode}.pt", map_location=DEVICE)
            )
            ...
            trials = json.loads(
                (MODEL_DIR / f"04_{role}_dnn_no_pca_bayesian_{label_mode}_trials.json").read_text(encoding="utf-8")
            )
    ...
    out_path = MODEL_DIR / "04_dnn_no_pca_bayesian_results.xlsx"
