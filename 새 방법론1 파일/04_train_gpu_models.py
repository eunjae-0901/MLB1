...
class InjuryDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.kind == "lstm": x = torch.cat([x, m[:, None]], dim=1)
...
def main(kind, seed, epochs):
    ...
    def loader(indices, shuffle=False):
        return DataLoader(InjuryDataset(x, mask, y, days, weight, indices, kind),
                          batch_size=256 if kind == "lstm" else 16, shuffle=shuffle,
                          num_workers=0, pin_memory=True)
    tr_idx, va_idx = np.flatnonzero(train), np.flatnonzero(val)
    model = (LSTMModel(x.shape[2]) if kind == "lstm" else ViTModel()).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4 if kind == "lstm" else 2e-5, weight_decay=1e-4)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, mb, yb, db, wb, _ in loader(tr_idx, True):
            ...
            cls = (F.binary_cross_entropy_with_logits(logits, yb, reduction="none") * wb).mean()
            positive = yb.eq(1); reg_loss = F.smooth_l1_loss(reg[positive], db[positive]) if positive.any() else cls * 0
            loss = cls + .30 * reg_loss; loss.backward(); nn.utils.clip_grad_norm_(model.parameters(), 1.0); optimizer.step(); total += loss.item()
        ...
        if score > best_score: best_score, best_state, wait = score, copy.deepcopy(model.state_dict()), patience
            ...
            wait -= 1
            if wait == 0: break
    model.load_state_dict(best_state); torch.save({"model":best_state,"mean":mean,"std":std}, RESULTS/f"{kind}_seed{seed}.pt")
    _, val_p, _ = predict(model, loader(va_idx), device); threshold = choose_threshold(y[va_idx], val_p)
    rows, predictions = [], []
...
if __name__ == "__main__":
    p=argparse.ArgumentParser(); p.add_argument("--model",choices=["lstm","vit"],required=True); p.add_argument("--seed",type=int,default=42); p.add_argument("--epochs",type=int,default=100); a=p.parse_args()
